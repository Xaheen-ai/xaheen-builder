/**
 * Xaheen SDK - Tenant ID Resolution Hook
 *
...
export function useSessionTenantId(appId?: string): TenantId | undefined {
    return useMemo(() => {
        try {
            ...
            if (appId) {
                const perApp = localStorage.getItem(`xaheen_${appId}_tenant_id`);
                if (perApp) return perApp as TenantId;
            }

            // 2. Scan all known app keys
            for (const knownId of KNOWN_APP_IDS) {
                const val = localStorage.getItem(`xaheen_${knownId}_tenant_id`);
                if (val) return val as TenantId;
            }

            // 3. Legacy key
            const legacy = localStorage.getItem("xaheen_tenant_id");
            ...
            for (const id of appIds) {
                const userJson = localStorage.getItem(`xaheen_${id}_user`);
                if (userJson) {
            ...
            // 5. Legacy user key
            const legacyUser = localStorage.getItem("xaheen_user");
            if (legacyUser) {
